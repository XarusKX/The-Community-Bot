...
module.exports.updateWebtoon = async (client) => {

    return new Promise(function (resolve, reject) {
        request("https://www.webtoons.com/en/dailySchedule", async function(err, res, body) {
            ...
            let bulkWebtoon = await client.db1.Webtoon.bulkCreate(webtoon, {
                updateOnDuplicate: true
            });

            try {
                console.log(`${client.config.cliColor("GREEN")} Success added! ${client.config.cliColor("NC")}`);
                return resolve(bulkWebtoon);
            } catch (error) {
...
module.exports.showWebtoon = async (client, msg, url) => {
    var comic = {
        title: "",
        author: "",
        link: "",
    ...
    return new Promise(function (resolve, reject) {
        request((client, msg, url), async function(err, res, body) {
            if (err && res.statusCode !== 200) return reject(new Error(err));
            let $ = cheerio.load(body);
...
exports.scheduleWebtoon = async (client, msg, arg) => {
    return new Promise(function(resolve, reject) {
        request("https://www.webtoons.com/en/dailySchedule", async function(err, res, body) {
            if (err && res.statusCode !== 200) return reject(new Error(err));
